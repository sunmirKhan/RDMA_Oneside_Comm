...
struct block blks[block_num];
struct rdma_buffer_attr blk_meta_mr_infos[block_num];
...
static void prepare_block_info()
{
	get_blocks(block_num, blks);
	for (int i = 0; i < block_num; i++) {
		struct ibv_mr *block_meta_info_mr = rdma_buffer_register(pd /* which protection domain */,
			&blks[i],
			sizeof(blks[i]) /* what size to allocate */,
			(IBV_ACCESS_LOCAL_WRITE |
				IBV_ACCESS_REMOTE_READ |
				IBV_ACCESS_REMOTE_WRITE) /* access permissions */);
		blk_meta_mr_infos[i].address= (uint64_t)block_meta_info_mr->addr;
		blk_meta_mr_infos[i].length = (uint32_t)block_meta_info_mr->length;
		blk_meta_mr_infos[i].stag.local_stag = (uint32_t)block_meta_info_mr->lkey;
	}
	server_metadata_mr = rdma_buffer_register(pd /* which protection domain*/,
		&blk_meta_mr_infos /* which memory to register */,
		sizeof(blk_meta_mr_infos) /* what is the size of memory */,
